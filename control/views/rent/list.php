...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'reorder') {
        ...
        if ($ids) {
            ...
            $stmt = $pdo->prepare('UPDATE rent_properties SET sort = :sort WHERE id = :id');
    ...
    $sql = 'SELECT id, name, cate, price, location, status, lastUpdateDate, sort FROM `rent_properties`';
    if ($where) {
...
<script>
  (function () {
    ...
    function sendOrder() {
      ...
      fetch('index.php?page=rent_list', {
        method: 'POST',
        body: formData,
